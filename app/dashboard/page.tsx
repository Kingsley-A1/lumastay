...
import { getBookings, getRoomBySlug } from "@/lib/data";
...
export default function DashboardPage() {
  ...
  const allBookings = getBookings().filter((b) => b.guestName === "John Smith");

  const filteredBookings = allBookings.filter((b) => {
    if (activeTab === "all") return true;
    if (activeTab === "upcoming") return [BookingStatus.Confirmed, BookingStatus.PendingPayment, BookingStatus.CheckedIn].includes(b.status as BookingStatus);
    if (activeTab === "completed") return [BookingStatus.Completed, BookingStatus.CheckedOut].includes(b.status as BookingStatus);
  ...
  const stats = {
    ...
    upcoming: allBookings.filter((b) => [BookingStatus.Confirmed, BookingStatus.PendingPayment, BookingStatus.CheckedIn].includes(b.status as BookingStatus)).length,
    completed: allBookings.filter((b) => [BookingStatus.Completed, BookingStatus.CheckedOut].includes(b.status as BookingStatus)).length,
